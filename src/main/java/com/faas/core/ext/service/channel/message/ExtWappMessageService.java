...
@Component
public class ExtWappMessageService {
    ...
    public String smsMessageBodyPopulate(SmsMessageDBModel smsMessageDBModel) throws IOException {
        ...
        if (smsMessageBody.contains(AppConstant.PWA_LINK_TAG)) {
            String pwaShortLink = smsPWAShortLinkCreate(smsMessageDBModel);
            if (pwaShortLink == null) {
                return null;
            }
            smsMessageBody = smsMessageBody.replace(AppConstant.PWA_LINK_TAG, pwaShortLink);
        }
